...
```
ansible nodes -i inventory/hosts.poc_acoss -m shell -a 'export no_proxy=10.0.0.0/8,.recouv; export http_proxy=http://squid.cnp.recouv:8080/;export https_proxy=http://squid.cnp.recouv:8080/; subscription-manager repos --disable="*" --enable="rhel-7-server-rpms" --enable="rhel-7-server-extras-rpms" --enable="rhel-7-server-ose-3.11-rpms" '
```
